...
async function processAdvancedSpeech(audioData: string, config: any) {
  ...
  // In reality, this would be actual speech recognition
  let phraseSet = businessPhrases;
  
  // Simple heuristic based on audio data length to vary responses
  const dataLength = audioData.length;
  if (dataLength % 3 === 0) {
    phraseSet = healthcarePhrases;
  } else if (dataLength % 5 === 0) {
    phraseSet// app/api/speech/to-text/route.ts - Google Cloud Speech-to-Text
export async function POST(request: Request) {
  try {
    const { audioData, config } = await request.json();
